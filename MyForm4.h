...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->DownloadMenuButton->Click += gcnew System::EventHandler(this, &MyForm4::DownloadMenuButton_Click);
			// 
			...
			this->обновитьToolStripMenuItem->Click += gcnew System::EventHandler(this, &MyForm4::обновитьToolStripMenuItem_Click);
			// 
			// AddMenuButton
			// 
			...
			this->AddMenuButton->Click += gcnew System::EventHandler(this, &MyForm4::AddMenuButton_Click);
			// 
			...
			this->удалитьToolStripMenuItem->Click += gcnew System::EventHandler(this, &MyForm4::удалитьToolStripMenuItem_Click);
			// 
			...
			this->выходToolStripMenuItem->Click += gcnew System::EventHandler(this, &MyForm4::выходToolStripMenuItem_Click);
			// 
			// MyForm4
			// 
...
#pragma endregion
	private: System::Void DownloadMenuButton_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		String^ query = "SELECT * FROM [Разнарядка]";	//запрос
		...
		if (dbReader->HasRows == false) {
			...
			while (dbReader->Read()) {
				dataGridView1->Rows->Add(dbReader["Код_авто"], dbReader["Код_водителя"], dbReader["Регистрационный_номер"], dbReader["Пункт_назначения"], dbReader["Путевой_лист"]);
			}
		}
...
private: System::Void обновитьToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	String^ fifth_string = dataGridView1->Rows[index]->Cells[3]->Value->ToString();
	...
	String^ query = "UPDATE [Разнарядка] SET Код_водителя = '" + second_string + "', Регистрационный_номер = '" + third_string + "', Пункт_назначения = '" + fourth_string + "', Путевой_лист = '" + fifth_string + "' WHERE Код_авто = " + first_string;
...
private: System::Void AddMenuButton_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	String^ query = "INSERT INTO [Разнарядка] VALUES (" + first_string + ", '" + second_string + "', '" + third_string + "', '" + fourth_string + "', '" + fifth_string + "')";
	OleDbCommand^ dbCommand = gcnew OleDbCommand(query, dbConnection);
...
private: System::Void удалитьToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	//выполняем запрос к БД
	dbConnection->Open();	//открываем соединение
	String^ query = "DELETE FROM [Разнарядка] WHERE Код_авто = " + first_string;	//запрос
	OleDbCommand^ dbCommand = gcnew OleDbCommand(query, dbConnection);	//команда
